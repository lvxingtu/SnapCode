...
static Value evalMathExpr(DebugApp anApp, ObjectReference anOR, JExprMath anExpr) throws Exception
{
    ...
    if(opCount==1) {
        if(op==JExprMath.Op.Not) {
            if(val1.type() instanceof BooleanType) { boolean val = ((PrimitiveValue)val1).booleanValue();
        ...
        if(op==JExprMath.Op.Negate) { // Need to not promote everything to double
            if(val1.type() instanceof PrimitiveType) { double val = ((PrimitiveValue)val1).doubleValue();
                return anApp._vm.mirrorOf(-val); }
    ...
    // Handle Binary
        ...
        switch(op) {
            ...
            case LessThanOrEqual: case GreaterThanOrEqual: return compareNumeric(anApp, val1, val2, op);
            case Or: case And: return compareLogical(anApp, val1, val2, op);
            default: throw new RuntimeException("Operator not supported " + anExpr.getOp());
