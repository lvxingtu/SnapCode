 ...
 */
public RunApp(WebURL aURL, String theArgs[])  { _url = aURL; setArgs(theArgs); }
...
public boolean setArgs(String argv[])
{
    ...
    // Iterate over args
    for(int i=0; i<argv.length; i++) { String token = argv[i];
        ...
        // Handle -classpath
            ...
            if(i==argv.length-1) { error("No classpath specified."); return false; }
            classPath = argv[++i];
            javaArgs += "-cp \"" + classPath + '"';
        }
        
        // Handle -help, -version or -[unknown]
        else if(token.equals("-help")) { error(""); return false; }
        else if(token.equals("-version")) { error("Version 1"); return false; }
        else if(token.startsWith("-")) { error("invalid option: " + token); return false; }
    ...
    // Configure context
    setVmArgs(javaArgs); setAppArgs(progArgs);
    setMainClassName(clsName); setClassPath(classPath);
 ...
 */
public void setMainClassName(String mainClassName)  { this._mainClassName = mainClassName; }
...
protected void startProcessReaders()
{
    ...
    _outReader.setPriority(Thread.MAX_PRIORITY-1); _outReader.start();
    _errReader = new StreamReader(_process.getErrorStream(), true);  // Start standard err
    _errReader.setPriority(Thread.MAX_PRIORITY-1); _errReader.start();
}
...
OutputListener _diagnostics = new OutputListener() {
    public void putString(String aStr) { System.out.println(aStr); }};
...
{
    InputStream   _is; boolean _isErr;
    
    StreamReader(InputStream anIS, boolean isErr)  { _is = anIS; _isErr = isErr; }
 ...
 */
public class Output {
    String _str; boolean _isErr;
    Output(String aStr, boolean isErr)  { _str = aStr; _isErr = isErr; }
