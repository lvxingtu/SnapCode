...
import snap.project.Breakpoint.Type;
import com.sun.jdi.*;
 ...
 */
public BreakpointReq(DebugApp aRuntime, Breakpoint aBP)  { _app = aRuntime; _bpoint = aBP; }

/**
 * Returns the name.
 */
public String getName()  { return _bpoint.getName(); }

/**
 * Returns the type.
 */
public Type getType()  { return _bpoint.getType(); }
...
protected void install(ReferenceType aRefType)
{
    ...
    catch(Exception exc) { _error = exc; _app.notifyError(this); }
}
 ...
 */
public EventRequest getEventRequest()  { return _request; }
    ...
    AmbiguousMethodException
{
    ...
    catch(AbsentInformationException e) { throw new LineNotFoundException(); }
    ...
    throws AmbiguousMethodException, NoSuchMethodException, NoSessionException
{
    ...
    Method firstMatch = null; int matchCount = 0; // > 1 implies overload
...
private static String normalizeArgTypeName(String aName, DebugApp anApp) throws NoSessionException
{
    ...
    if(aName.indexOf('.')==-1 || aName.startsWith("*.")) {
        ...
        } catch (IllegalArgumentException e) { } // We'll try the name as is 
    }
 ...
 */
public String toString()  { return _bpoint + " (" + getStatus() + ")"; }
