 ...
 */
public static JavaParser getShared()  { return _shared; }

/**
 * Returns the shared expression parser.
 */
public Parser getExprParser()  { return _ep!=null? _ep : (_ep=new Parser(_shared.getRule("Expression"))); }

/**
 * Returns the shared expression parser.
 */
public Parser getStmtParser()  { return _sp!=null? _sp : (_sp=new Parser(_shared.getRule("Statement"))); }
...
public Parser getImportsParser()
{
    ...
    Parser ip = new JavaParser(); ip.setRule(ip.getRule("JavaFileImports"));
    return _ip=ip;
}
...
public JFile getJavaFile(CharSequence anInput)
{
    ...
    // Get parse node
    ParseNode node = null; try { node = anInput!=null && anInput.length()>0? parse(anInput) : null; }
    catch(ParseException e) { if(_exception==null) _exception = e; }
    catch(Exception e) { _exception = e;
        Token t = getToken(); if(t!=null) System.err.println("Exeption at line " + (t.getLineIndex() + 1));
        e.printStackTrace(); }
    
    // Set JFile
    JFile jfile = node!=null? node.getCustomNode(JFile.class) : null; if(jfile==null) jfile = new JFile(); 
 ...
 */
public JavaTokenizer getTokenizer()  { return (JavaTokenizer)super.getTokenizer(); }

/**
 * Returns the tokenizer.
 */
protected Tokenizer createTokenizerImpl()  { return new JavaTokenizer(); }
...
public static class JavaTokenizer extends Tokenizer {
    ...
    public JavaTokenizer()  { setReadSingleLineComments(true); setReadMultiLineComments(true); }

    /** Returns a token from the current char to multi-line comment termination or input end. */
    public Token getMultiLineCommentTokenMore(Token aSpclTkn) { return super.getMultiLineCommentTokenMore(aSpclTkn); }
}
...
public static class EnumDeclHandler extends JNodeParseHandler <JClassDecl>
{
    ...
    { JClassDecl cd = new JClassDecl(); cd.setClassType(JClassDecl.ClassType.Enum); return cd; }
}
...
public static class ConstrDeclHandler extends JNodeParseHandler <JConstrDecl>
{
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        // Handle BlockStatement start "{"
            ...
            block.setStartToken(aNode.getStartToken()); block.setEndToken(aNode.getEndToken());
...
public static class ConstrCallHandler extends JNodeParseHandler <JConstrCall>
{
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        // Handle "this"/"super"
            ...
            id.setStartToken(aNode.getStartToken()); id.setEndToken(aNode.getEndToken());
            getPart().addId(id);
        }
...
{
    /** ParseHandler method. */
    protected void parsedOne(ParseNode aNode, String anId)  { getPart(); }
}
...
public static class ForStatementHandler extends JNodeParseHandler<JStmtFor>
{
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        if(anId=="Type") {
            JType type = aNode.getCustomNode(JType.class);
            JStmtVarDecl svd = new JStmtVarDecl(); svd.setType(type);
    ...
    public JStmtFor parsedAll()  { partIndex = 0; return super.parsedAll(); }
}
...
public static class IdentifierHandler extends JNodeParseHandler <JExprId> {
    ...
    /** ParseHandler method. */
    protected void parsedOne(ParseNode aNode, String anId)  { getPart().setName(aNode.getString()); }
}
...
public static abstract class BinaryExprHandler extends ParseHandler <JExpr> {
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        if(aNode.getCustomNode() instanceof JExpr) {
            ...
            else { _part = new JExprMath(_op, _part, part); _op = null; }
        }
...
public static class UnaryExprHandler extends JNodeParseHandler <JExpr>
{
    ...
    public JExpr parsedAll()  { _op = null; return super.parsedAll(); }
}
...
public static class InstanceOfExprHandler extends JNodeParseHandler <JExpr>
{
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        if(anId=="Type") {
            ...
            ie.setExpr(_part); ie.setType(aNode.getCustomNode(JType.class)); _part = ie;
        }
    }
}
...
public static class PrimaryPrefixHandler extends JNodeParseHandler <JExpr>
{
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        // Handle Name
            ...
            if(namePrime instanceof JExprChain) { JExprChain nameChain = (JExprChain)namePrime;
...
public static class PrimarySuffixHandler extends JNodeParseHandler <JExpr>
{
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        // Handle ("." | "::") Identifier
            ...
            if(_methodRef) { _part = new JExprMethodRef(null, id); _methodRef = false; }
            else _part = id;
        }
...
public static class LambdaExprHandler extends JNodeParseHandler <JExprLambda>
{
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        // Handle Identifier
        if(anId=="Identifier") {
            JVarDecl vd = new JVarDecl(); vd.setId(aNode.getCustomNode(JExprId.class));
            getPart().addParam(vd);
        }
...
public static class LiteralHandler extends JNodeParseHandler <JExprLiteral> {
    ...
    protected void parsedOne(ParseNode aNode, String anId)
    {
        ...
        else if(anId=="IntegerLiteral") { int len = s.length(); char c = s.charAt(len-1);
        ...
        // Handle FloatLiteral
        else if(anId=="FloatLiteral") { int len = s.length(); char c = s.charAt(len-1);
