...
{
    _file = aFile; _kind = aKind; _text = theText;
    _line = aLine; _column = aColumn; _start = aStart; _end = anEnd;
    return this;
}
 ...
 */
public boolean isError()  { return getKind()==Kind.Error; }
 ...
 * Returns the end char index.
 */
public void setEnd(int anEnd)  { _end = anEnd; }
...
{
    Kind k1 = getKind(), k2 = aBI.getKind(); if(k1!=k2) return k1.ordinal()-k2.ordinal();
    int comp = getFile().compareTo(aBI.getFile()); if(comp!=0) return comp;
    int l1 = getLine(), l2 = aBI.getLine(); if(l1!=l2) return l2-l1;
    int s1 = getStart(), s2 = aBI.getStart(); if(s1!=s2) return s2 - s1;
 ...
 */
public String toString()  { return String.format("%s:%d: %s", getFile().getPath(), getLine()+1, getText()); }
