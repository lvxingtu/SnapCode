...
{
    _site = aSite;
    String urls = getRemoteURLString(aSite); if(urls!=null) _remoteURL = WebURL.getURL(urls);
}
...
{
    WebFile cloneDir = getSite().getSandbox().getFile("Remote.clone");
 ...
 */
public WebSite getCloneSite()  { return getCloneDir().getURL().getAsSite(); }

/**
 * Returns the repository site for remote URL (defaults to RemoteSite).
 */
public WebSite getRepoSite()  { return getRemoteSite(); }
...
private WebFile getFile(WebSite aSite, String aPath, boolean doCreate, boolean isDir)
{
    ...
    WebFile file = aSite.getFile(aPath);
...
{
    List <WebFile> files = new ArrayList(); getUpdateFiles(getSite().getRootDir(), files);
 ...
 */
public boolean isModified(WebFile aFile)  { return getStatus(aFile)!=Status.Identical; }
...
protected Set <WebFile> getChangedFiles(WebFile aFile, Set <WebFile> theFiles)
{
    ...
    if(aFile.isDir() && !isIgnoreFile(aFile)) {
        ...
        if(cfile!=null) for(WebFile file : cfile.getFiles()) {
            if(fsite.getFile(file.getPath())==null)
...
public void commitFiles(List <WebFile> theLocalFiles, String aMessage, TaskMonitor aTM) throws Exception
{
    ...
    for(WebFile file : theLocalFiles) {
        ...
        catch(AccessException e) { ClientUtils.setAccess(e.getSite()); commitFile(file, aMessage); }
        aTM.endTask();
    }
}
...
public void updateFiles(List <WebFile> theLocalFiles, TaskMonitor aTM) throws Exception
{
    ...
    for(WebFile file : theLocalFiles) {
        ...
        if(aTM.isCancelled()) break;
        try { updateFile(file); }
        catch(AccessException e) { ClientUtils.setAccess(e.getSite()); updateFile(file); }
        aTM.endTask();
    }
}
 ...
 */
public void fileRemoved(WebFile aFile)  { setStatus(aFile, null); }
...
{
    WebFile file = aSite.getSandbox().getFile("/settings/remote"); if(file==null) return null;
    String text = file.getText(); Scanner scanner = new Scanner(text);
    String urls = scanner.hasNext()? scanner.next() : null; scanner.close();
    return urls;
}
...
public static void setRemoteURLString(WebSite aSite, String aURLS)
{
    ...
    WebFile file = aSite.getSandbox().getFile("/settings/remote");
    if(file==null) file = aSite.getSandbox().createFile("/settings/remote", false);
    try {
        if(aURLS==null || aURLS.length()==0) { if(file.getExists()) file.delete(); }
        else { file.setText(aURLS); file.save(); }
    }
...
{
    String urls = getRemoteURLString(aSite); if(urls!=null) urls = urls.toLowerCase();
