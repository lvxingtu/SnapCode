 ...
 */
public RunConfigs(WebSite aSite)  { _site = aSite; }

/**
 * Returns the primary RunConfig (or null, if none).
 */
public RunConfig getRunConfig()  { List <RunConfig> cfs = getRunConfigs(); return cfs.size()>0? cfs.get(0) : null; }

/**
 * Returns the RunConfigs.
 */
public List <RunConfig> getRunConfigs()  { return _runConfigs!=null? _runConfigs : (_runConfigs=readFile()); }
...
public void writeFile()
{
    ...
    for(int i=0, iMax=rconfs.size(); i<iMax; i++) { RunConfig rconf = rconfs.get(i);
 ...
 */
protected Settings getSettings()  { return Settings.get(getFile()); }
...
{
    WebFile file = _site.getSandbox().getFile("/settings/run_configs");
