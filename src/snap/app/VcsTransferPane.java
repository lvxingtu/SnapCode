...
public class VcsTransferPane extends ViewOwner {
    ...
    List <WebFile>      _files = new ArrayList();
...
public boolean showPanel(VcsPane aVC, List <WebFile> theFiles, Op anOp)
{
    ...
    _files = theFiles; _op = anOp;

    // If no transfer files, just tell user and return
    if(_files.size()==0) {
        String msg = "No " + getOp() + " files to transfer.", title = "Synchronize Files";
        DialogBox dbox = new DialogBox(title); dbox.setWarningMessage(msg);
        dbox.showMessageDialog(aVC._appPane.getUI());
        return false;
    }
    
    // Show confirmation dialog with files to transfer
    String mode = getOp().toString(), options[] = new String[] { mode, "Cancel" };
    DialogBox dbox = new DialogBox(mode + " Files Panel"); dbox.setContent(getUI()); dbox.setOptions(options);
    if(dbox.showOptionDialog(aVC._appPane.getUI(), mode)!=0) return false;
    
    // If commit, get message
    if(anOp==Op.Commit) {
        _commitMsg = getViewText("CommentText"); if(_commitMsg!=null) _commitMsg = _commitMsg.trim();
        if(_commitMsg==null || _commitMsg.length()==0) {
            DialogBox db = new DialogBox("Commit Files Message Panel"); db.setMessage("Enter Commit Message");
...
protected void initUI()
{
    ...
    if(getOp()!=Op.Commit) {
        ...
        getView("SplitView", SplitView.class).removeChild(sview);
    }
}
...
protected View getFileGraphic(WebFile aFile)
{
    ...
    if(!aFile.getExists()) badge = isCommit? RemovedLocalBadge : AddedRemoteBadge;
    else if(remoteFile==null) badge = isCommit? AddedLocalBadge : RemovedRemoteBadge;
    ...
    ImageView iview = new ImageView(image), bview = new ImageView(badge);
    StackView spane = new StackView(); spane.setAlign(Pos.TOP_LEFT); spane.setChildren(iview, bview);
    spane.setPrefSize(16+6,16); bview.setLean(Pos.CENTER_RIGHT);
    return spane;
}
