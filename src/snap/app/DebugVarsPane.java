 ...
 */
public DebugVarsPane(AppPane anAppPane)  { _appPane = anAppPane; }
 ...
 */
void resetVarTable()  { if(isUISet()) { _resetVarTable = true; resetLater(); } }
...
List createVarItems()
{
    ...
    DebugApp dapp = getDebugApp(); if(dapp==null) return Collections.EMPTY_LIST;
    StackFrame frame = dapp.getCurrentFrame(); if(frame==null) return Collections.EMPTY_LIST;
    ...
    List <LocalVariable> vars; try { vars = frame.visibleVariables(); }
    catch(Exception e) { System.err.println(e.getMessage()); vars = Collections.emptyList(); }
    for(LocalVariable lv : vars) {
        String name = null; Value value = null;
        try { name = lv.name(); value = frame.getValue(lv); } catch(Exception e) { name = e.toString(); }
...
public static class VarTreeItem implements Comparable <VarTreeItem> {
    ...
    // Ivars
    DebugApp _app; String _name; Object _value; VarTreeItem _parent; VarTreeItem _children[];
    
    /** Create VarTableItem. */
    public VarTreeItem(DebugApp anApp, String aName, Object aValue)  { _app = anApp; _name = aName; _value = aValue; }
    ...
    {
        Object value = _value;
        if(value instanceof ArrayReference) { ArrayReference aref = (ArrayReference)value;
            ReferenceType rtype = aref.referenceType();
            String name = rtype.name(); int ind = name.lastIndexOf('.'); if(ind>0) name = name.substring(ind+1);
        ...
        if(value instanceof ObjectReference) { ObjectReference oref = (ObjectReference)value;
    ...
    public String getVarClass()
    {
        ...
        Object v = getValue(); return v!=null? v.getClass().getName() : null;
    }
    ...
    {
        if(_name=="this") return -1; if(anItem._name=="this") return 1;
...
public static class VarTreeResolver extends TreeResolver <VarTreeItem> {
    ...
    public VarTreeItem[] getChildren(VarTreeItem aParent)  { return aParent.getChildren(); }
