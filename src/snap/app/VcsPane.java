...
{
    _sitePane = aSitePane; _site = _sitePane.getSite(); _site.setProp(VcsPane.class.getName(), this);
 ...
 * Sets the anAppPane.
 */
public void setAppPane(AppPane anAP)  { _appPane = anAP; }
...
protected void initUI()
{
    ...
    _remoteBrowser = new WebBrowser(); _remoteBrowser.setAutosizing("-~-,-~-");
...
public void resetUI()
{
    ...
    // Update ProgressBar
    ProgressBar pb = getView("ProgressBar", ProgressBar.class); boolean loading = _remoteBrowser.isLoading();
...
public void openSite()
{
    ...
    DialogBox dbox = new DialogBox("Checkout Project Files"); dbox.setMessage(msg);
...
{
    try { _vc.disconnect(new TaskMonitor.Text(System.out)); }
    catch(Exception e)  {
        DialogBox db = new DialogBox("Disconnect Error"); db.setErrorMessage(e.toString());
...
public void checkout()
{
    TaskRunner runner = new TaskRunnerPanel(_appPane.getUI(), "Checkout from " + _vc.getRemoteURLString()) {
        ...
        public Object run() throws Exception
        {
            ...
            _vc.checkout(this); return null;
        }
        public void success(Object aRes) { checkoutSuccess(_oldAutoBuildEnabled); }
...
protected void checkoutSuccess(boolean oldAutoBuildEnabled)
{
    ...
    Project proj = Project.get(getSite()); if(proj!=null) proj.readSettings();
...
public void commitFiles(List <WebFile> theFiles)
{
    ...
    if(!fpane.showPanel(this, sfiles, Op.Commit)) return;
...
{
    // Create list
    List <WebFile> fromFiles = theFromFiles!=null? theFromFiles : (List)Arrays.asList(getSite().getRootDir());
    List <WebFile> xfiles = new ArrayList();
    
    try { for(WebFile file : fromFiles) _vc.getCommitFiles(file, xfiles); }
    catch(AccessException e) { if(ClientUtils.setAccess(e.getSite())) return getCommitFiles(theFromFiles); throw e; }
    
    // Sort files and return
    Collections.sort(xfiles); return xfiles;
}
...
protected void commitFilesImpl(final List <WebFile> theFiles, final String aMessage)
{
    ...
    new TaskRunnerPanel(_appPane.getUI(), "Commit files to remote site") {
        public Object run() throws Exception  { _vc.commitFiles(theFiles, aMessage, this); return null; }
...
public void updateFiles(List <WebFile> theFiles)
{
    ...
    // Run VersionControlFilesPane for files and op
    if(!new VcsTransferPane().showPanel(this, sfiles, Op.Update)) return;
...
protected List <WebFile> getUpdateFiles(List <WebFile> theFromFiles)
{
    ...
    try { for(WebFile file : fromFiles) _vc.getUpdateFiles(file, xfiles); }
    catch(AccessException e) { if(ClientUtils.setAccess(e.getSite())) return getUpdateFiles(theFromFiles); throw e; }
    catch(Exception e) { DialogBox db = new DialogBox("Disconnect Error"); db.setErrorMessage(e.toString());
        db.showMessageDialog(_appPane.getUI()); }
...
protected void updateFilesImpl(final List <WebFile> theFiles)
{
    ...
    new TaskRunnerPanel(_appPane.getUI(), "Update files from remote site") {
        public Object run() throws Exception  { _vc.updateFiles(theFiles, this); return null; }
        public void success(Object anObj)
        {
...
public void replaceFiles(List <WebFile> theFiles)
{
    ...
    // Run VersionControlFilesPane for files and op
    if(!new VcsTransferPane().showPanel(this, sfiles, Op.Replace)) return;
...
protected List <WebFile> getReplaceFiles(List <WebFile> theFromFiles)
{
    ...
    try { for(WebFile file : fromFiles) _vc.getReplaceFiles(file, xfiles); }
    catch(AccessException e) { if(ClientUtils.setAccess(e.getSite())) return getReplaceFiles(theFromFiles); throw e; }
...
protected void replaceFilesImpl(final List <WebFile> theFiles)
{
    ...
    new TaskRunnerPanel(_appPane.getUI(), "Replace files from remote site") {
        public Object run() throws Exception  { _vc.replaceFiles(theFiles, this); return null; }
        public void success(Object anObj)
        {
            for(WebFile file : theFiles) file.reload();
            for(WebFile file : theFiles) _appPane.getBrowser().reloadFile(file); // Refresh replaced files
 ...
 */
void fileAdded(WebFile aFile)  { _vc.fileAdded(aFile); }

/**
 * Called when file removed from project.
 */
void fileRemoved(WebFile aFile)  { _vc.fileRemoved(aFile); }

/**
 * Called when file saved in project.
 */
void fileSaved(WebFile aFile)  { _vc.fileSaved(aFile); }

/**
 * Returns the VersionControlPane for given project.
 */
public synchronized static VcsPane get(WebSite aSite)  { return (VcsPane)aSite.getProp(VcsPane.class.getName()); }
