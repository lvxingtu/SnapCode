 ...
 */
public static SnapEditorPopup getShared()  { return _s!=null? _s : (_s=new SnapEditorPopup()); } static SnapEditorPopup _s;
...
public void activatePopupList(JNodeView aPart, String aString, int anIndex)
{
    ...
    JNode node = null; try { node = _exprParser.parseCustom(aString, JNode.class); } catch(Exception e) { }
    ...
    if(!(id instanceof JExprId))  { hide(); return; }
    
    // If same Id, just return
    if(SnapUtils.equals(id.getName(), _idText)) return; _idText = id.getName();

    // Get suggestions
    JavaDecl suggestions[] = JavaSuggestion.getSuggestions(id);
 ...
 * Hides the popup.
 */
public void hide()  { getPopup().hide(); _idText = null; }

/**
 * Returns whether popup is visible.
 */
public boolean isShowing()  { return getPopup().isShowing(); }

/**
 * Returns the popup.
 */
public PopupWindow getPopup()  { return _popup!=null? _popup : (_popup=createPopup()); }
...
{
    _listView = new ListView(); _listView.setFont(Font.Arial11);
