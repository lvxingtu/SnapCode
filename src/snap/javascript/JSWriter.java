...
/**
 * A class to convert Java to JavaScript.
 */
...
public void writeJNode(JNode aNode)
{
    ...
    else append("JSWriter: write" + aNode.getClass().getSimpleName() + " not implemented");
}
...
public void writeJClassDecl(JClassDecl aCDecl)
{
    ...
    // Add export call
    //if(aCDecl.getFile().getPackageName()!=null)  append("export ");
    ...
    //append(mstr); append(isInterface? "interface " : "class "); append(cname).append(' ');
    ...
    //for(JType etyp : etypes) { writeJType(etyp); if(etyp!=elast) append(", "); else append(' '); }
    ...
    //for(JType ityp : itypes) { writeJType(ityp); if(ityp!=ilast) append(", "); else append(' '); }
    
    // Declare class
    append("var ").append(cname).append(" = (function () {").endln().endln().indent();
    
    // Declare constructor
    append("function ").append(cname).append("() {").endln();
    append("}").endln().endln();
    ...
    // Return class
    append("return ").append(cname).append(";").endln();
        
    // Terminate
    outdent(); endln();
    append("}").append("());").endln().endln();
    //.append("(").append(cname).append(" || (").append(cname).append(" = {}));").endln().endln();
    
    // Write class decl
    append(cname).append("[\"__class\"] = \"").append(cname).append("\";").endln().endln();
    
    // If main, trigger it
    //if(aCDecl.getMethodDecl("main", new Class[] { String[].class })!=null)
    //    append(cname).append(".main(null);").endln();
...
public void writeJConstrDecls(JConstrDecl theCDecls[])
{
    ...
    JSWriterUtils.sort(theCDecls);
    JModifiers mods = theCDecls[0].getMods();
    JVarDecl params[] = JSWriterUtils.getCommonParams(theCDecls);
    ...
    List <String> dispatchConds = JSWriterUtils.getMethodDispatchConditionals(theCDecls, params, false);
    ...
    for(int i=0,iMax=theCDecls.length;i<iMax;i++) { JConstrDecl cd = theCDecls[iMax-i-1];
        ...
        for(int j=0,jMax=cdparams.size();j<jMax;j++) { JVarDecl vd = cdparams.get(j);
            if(!vd.getName().equals(params[j].getName())) {
                String lvar = "var " + vd.getName() + " = " + params[j].getName() + ";";
...
public void writeJMethodDecl(JMethodDecl aMDecl)
{
    ...
    String cname = aMDecl.getEnclosingClassDecl().getSimpleName();
    append(cname).append(".");
    if(!aMDecl.getMods().isStatic()) append("prototype.");

    writeJMethodDeclHead(mods, name, rtype, params, false);
    
    // If interface or abstract, just write semi-colon
    //if(aMDecl.getEnclosingClassDecl().isInterface() || mods!=null && mods.isAbstract()) append(';').endln();
    
    // Otherwise, write method block
    //else
    writeJStmtBlock(aMDecl.getBlock(), true);
}
    ...
    boolean isCombined)
{
    ...
    //writeJModifiers(theMods); append(aName).append("(");
    
    append(aName).append(" = function (");
    ...
    for(JVarDecl p : theParams) {
        ...
        //append(" : ");
        //String tstr = JSWriterUtils.getTypeString(p.getType()); append(tstr);
        if(p!=plast) append(", ");
    }
    
    // Write params close char and return type (if not empty/void)
    append(") ");
    //String tstr = aReturnType!=null? JSWriterUtils.getTypeString(aReturnType) : "";
...
public void writeJMethodDecls(JMethodDecl theMDecls[])
{
    ...
    JSWriterUtils.sort(theMDecls);
    JMethodDecl md0 = theMDecls[0];
    JModifiers mods = md0.getMods();
    String name = md0.getName();
    JType rtype = JSWriterUtils.getCommonReturnType(theMDecls);
    JVarDecl params[] = JSWriterUtils.getCommonParams(theMDecls);
    ...
    List <String> dispatchLines = JSWriterUtils.getMethodDispatchConditionals(theMDecls, params, true);
    ...
    for(JMethodDecl md : theMDecls) {
        String mname = JSWriterUtils.getMethodNameUnique(md);
...
public void writeJVarDecl(JVarDecl aVD)
{
    ...
    // Write type
    //JType type = aVD.getType(); String tstr = JSWriterUtils.getTypeString(type);
...
{
    String str = JSWriterUtils.getTypeString(aType); if(str.length()==0) return;
    append(str);
}
...
public void writeJStmt(JStmt aStmt)
{
    ...
    else throw new RuntimeException("JSWriter.writeJStmt: Unsupported statement " + aStmt.getClass());
...
public void writeJStmtFor(JStmtFor aStmt)
{
    ...
    // Handle for(each)
    if(aStmt.isForEach()) {
        append("var ").append(initVD.getName()).append(" of ");
        writeJExpr(cond);
    }
    ...
    // Handle conventional for()
        ...
        if(initVD!=null) {
            if(initVD.getType()!=null) append("var ");
...
public void writeJExpr(JExpr aExpr)
{
    ...
    else throw new RuntimeException("JSWriter.writeJExpr: Unsupported expression " + aExpr.getClass());
...
public void writeJExprId(JExprId aExpr)
{
    ...
    //    if(decl.isStatic()) { if(!decl.getClassName().startsWith("jsweet.")) append(decl.getClassSimpleName()).append('.'); }
    //    else append("this.");  }
...
public void writeJExprMath(JExprMath aExpr)
{
    ...
    else throw new RuntimeException("JSWriter.writeJExprMath: Unsupported op: " + op);
}
...
{
    System.out.println("JSWriter: Need to write method ref: " + aExpr.getFile().getEvalClassName());
}
 ...
 * Append String.
 */
public JSWriter append(String aStr)  { cd(); _sb.append(aStr); return this; }

/**
 * Append char.
 */
public JSWriter append(char aValue)  { cd(); _sb.append(aValue); return this; }

/**
 * Append Int.
 */
public JSWriter append(int aValue)  { cd(); _sb.append(aValue); return this; }

/**
 * Append Double.
 */
public JSWriter append(double aValue)  { cd(); _sb.append(aValue); return this; }

/**
 * Append newline.
 */
public JSWriter endln()  { _sb.append('\n'); _lineStart = true; return this; }

/**
 * Append indent.
 */
public JSWriter indent()  { _indent++; return this; }

/**
 * Append indent.
 */
public JSWriter outdent()  { _indent--; return this; }

/**
 * Append indent.
 */
public JSWriter appendIndent()  { for(int i=0;i<_indent;i++) _sb.append(_indentStr); return this; }
