...
protected void paintFront(Painter aPntr)
{
    ...
    for(Marker m : getMarkers()) {
        m.setY(m._y/th*h);
        aPntr.setPaint(m.getColor()); aPntr.fill(m);
        aPntr.setPaint(m.getStrokeColor()); aPntr.draw(m);
    }
}
...
public abstract static class Marker <T> extends Rect {
    ...
    public Marker(T aTarget)  { _target = aTarget; setRect(2,0,9,5); }
...
public class TokenMarker extends Marker <TextBoxToken> {
    ...
    /** Returns the selection start. */
    public int getSelStart()  { return _target.getLine().getStart() + _target.getStart(); }
