 ...
 */
public JStmtBlock getTryBlock()  { return _tryBlock; }

/**
 * Sets the try block.
 */
public void setTryBlock(JStmtBlock aBlock)  { replaceChild(_tryBlock, _tryBlock=aBlock); }

/**
 * Returns the catch blocks.
 */
public List <CatchBlock> getCatchBlocks()  { return _catchBlocks; }

/**
 * Adds a catch block.
 */
public void addCatchBlock(CatchBlock aBlock)  { _catchBlocks.add(aBlock); addChild(aBlock, -1); }
 ...
 */
public void setFinallyBlock(JStmtBlock aBlock)  { replaceChild(_finallyBlock, _finallyBlock=aBlock); }
...
public void addStatementBlock(JStmtBlock aBlock)
{
    ...
    if(_tryBlock==null) { setTryBlock(aBlock); return; }
    
    // If last CatchBlock doesn't have StatementBlock, set it
    int ccount = _catchBlocks.size(); if(ccount>0) { CatchBlock lcatch = _catchBlocks.get(ccount-1);
        if(lcatch.getBlock()==null) {
            lcatch.setBlock(aBlock); return; }}
...
public static class CatchBlock extends JNode {
    ...
    /** Sets the parameter. */
    public void setParameter(JVarDecl aVD)  { replaceChild(_param, _param=aVD); }
